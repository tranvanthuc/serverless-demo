...
app.post("/pokemon/", (req, res) => {
  ...
  const query = `INSERT INTO pokemons (name, height, weight, avatar) VALUES ('${name}', '${height}', '${weight}', '${avatar}')`;
  pool.query(query, (err, results, fields) => {
    if (err) {
      const response = { data: null, message: err.message };
      res.send(response);
...
app.put("/pokemon/:id", (req, res) => {
  const { id } = req.params;
  const query = `SELECT * FROM pokemons WHERE id=${id} LIMIT 1`;
  pool.query(query, (err, results, fields) => {
    if (err) {
      const response = { data: null, message: err.message };
      res.send(response);
    ...
    const query = `UPDATE pokemons SET name='${name}', height='${height}', weight='${weight}', avatar='${avatar}' WHERE id='${id}'`;
    pool.query(query, (err, results, fields) => {
      if (err) {
        const response = { data: null, message: err.message };
        res.send(response);
